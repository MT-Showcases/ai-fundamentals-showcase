...
export const ch05: Chapter = {
    ...
    sections: [
      ...
      { title: 'Lab 2 — Reti Neurali con MNIST', content: 'Costruisci e addestra la tua prima rete neurale in Python. Scarica ZIP Lab 2 (`ml-lab-02-neural-networks.zip`), installa le dipendenze con `pip install -r requirements.txt` (include numpy, scikit-learn, matplotlib, seaborn) ed esegui `python main.py`. Il lab usa il dataset MNIST — 10.000 immagini di cifre scritte a mano (28x28 pixel = 784 feature) — e addestra un MLP (Multi-Layer Perceptron) via Scikit-learn con un singolo hidden layer da 50 neuroni. Alla fine viene visualizzata una griglia 2x5: titoli in verde = predizioni corrette, in rosso = errori. Sperimenta modificando `hidden_layer_sizes=(50,)` in `hidden_layer_sizes=(50, 50,)` per aggiungere profondità e osservare l\'impatto su tempo di training e accuracy.', media: [ { type: 'infographic', title: 'Infografica — Architettura MNIST', description: 'Schema visuale: 784 input → 50 hidden → 10 output. Flusso backpropagation sul dataset MNIST.', placeholderPath: 'media/ch05-neural-networks/lab/infographic.jpg', notes: 'ready' } ] },
    ],
    keyTakeaways: [
